...
{
  const year = document.getElementById("year_list").value;
  const boxes = document.querySelectorAll('.column2');
  console.log(boxes);
  ...
  for (const item in temp[0]) {
    if(i>4){
      ...
      clone=individual.cloneNode(true);
      // console.log(clone);
      clone.children[0].children[0].id='er'+i;
      card.append(clone);
    }
    
 
    // document.getElementById('er'+i).innerHTML= `${item}${parseFloat(temp[0][item]).toFixed(2)}`;
